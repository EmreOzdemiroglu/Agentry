...
func loadAndRunChain() {
	...
	if len(savedChains) == 0 {
		fmt.Println("No saved chains (.json files) found in the current directory.")
		return
	}

	fmt.Println("Available Chains:")
	for i, chainFile := range savedChains {
		// Display without the .json extension for cleaner look
		chainName := strings.TrimSuffix(chainFile, ".json")
		fmt.Printf("  %d: %s (%s)\n", i+1, chainName, chainFile)
	}
	...
	filename := savedChains[selectedIndex]
	...
	for {
		...
		case "E":
			edited := editChain(&loadedChain, filename)
			if edited {
				fmt.Println("Chain updated. Returning to options for this chain.")
				// Loop continues, showing the Run/Edit/Back menu again for the modified chain
			} else {
...
// findSavedChains searches for .json files in the current directory
...
func loadChain(filename string) (AgentChain, error) {
	var chain AgentChain
	fileData, err := os.ReadFile(filename)
	if err != nil {
		return chain, fmt.Errorf("failed to read file %s: %w", filename, err)
	}

	err = json.Unmarshal(fileData, &chain)
	if err != nil {
		return chain, fmt.Errorf("failed to unmarshal JSON from %s: %w", filename, err)
	}

	// Optional: Validate the loaded chain structure if needed
	if chain.Name == "" || len(chain.Agents) == 0 {
		// Basic validation: ensure it has a name and at least one agent
		// You might add more checks here (e.g., non-empty model names)
		// For now, we can just warn or proceed
...
// saveChain marshals the AgentChain to JSON and saves it to a file
func saveChain(chain AgentChain, filename string) error {
	jsonData, err := json.MarshalIndent(chain, "", "  ") // Use MarshalIndent for readability
	if err != nil {
		return fmt.Errorf("failed to marshal chain to JSON: %w", err)
	}

	err = os.WriteFile(filename, jsonData, 0644) // 0644 provides read/write for owner, read for others
	if err != nil {
		return fmt.Errorf("failed to write chain to file '%s': %w", filename, err)
	}
...
// saveRunLog saves the captured log buffer to a timestamped markdown file
func saveRunLog(chain AgentChain, logBuffer *bytes.Buffer, startTime time.Time) error {
	...
	sanitizedName = strings.ReplaceAll(sanitizedName, "/", "-") // Avoid path separators
	// Add more sanitization if needed (remove special chars)

	filename := fmt.Sprintf("chatruns_%s_%s.md",
		startTime.Format("20060102_150405"), // YYYYMMDD_HHMMSS format
		sanitizedName)

	err := os.WriteFile(filename, logBuffer.Bytes(), 0644)
	if err != nil {
		return fmt.Errorf("failed to write log file '%s': %w", filename, err)
	}
	return nil
}
...
func editChain(chain *AgentChain, originalFilename string) bool {
	fmt.Printf("\n--- Editing Chain: %s ---\n", chain.Name)
	...
	for {
		...
		case "S":
			...
			err := saveChain(*chain, originalFilename) // Overwrite original file
			if err != nil {
